...
const Manager = () => {
  ...
  const deletepass = async (idToDelete) => {
    try {
      await fetch(`http://localhost:3000/passwords/${idToDelete}`, {
        method: "DELETE",
      });
      getpasswords();
  ...
  // Save or update password to backend with detailed error logging
  const savepass = async () => {
    if (
      ...
      try {
        ...
        const url = editingId
          ? `http://localhost:3000/passwords/${editingId}`
        ...
        console.error(err);
        toast.error("Failed to save/update password: " + err.message);
      }
    } else {
